...
class AdaptFs(MagOperation):
    ...
    def _build_new_dir(self, bname, old_path):
        ...
        if self._path_style == "fascicle":
            m = re.match(
                "^(?P<bid>.+?)" "_" "(?P<year>[0-9]{4})" "(?P<month>[0-9]{2})" "(?P<day>[0-9]{2})" "_[0-9]+" "_" "(?P<issue>[0-9]+)(?P<supll>.+)$",
                bname,
            )
            if not m:
            ...
            return str(
                ...
                / m.group("year")
                / m.group("month")
                / m.group("issue").zfill(4)
       #         / (m.group("year") + m.group("month") +  m.group("issue"))
    ...
    def _build_new_filename(self, old_path: MagResourcePath, index, bname) -> str:
        if self._short_filenames:
            ...
            file_basename = f"{bname}_{index:04d}"
