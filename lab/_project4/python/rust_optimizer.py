...
# Try to import psutil for better process cleanup on Windows
try:
    import psutil
except ImportError:
    psutil = None
...
def cleanup_process():
    ...
    if current_process and current_process.poll() is None:
        try:
            # On Windows, try to kill child processes as well
            if os.name == 'nt' and psutil is not None:
                try:
                    ...
                    print(json.dumps({"type": "warning", "message": f"psutil cleanup failed: {e}"}), flush=True)
